...
public class NPC : MonoBehaviour, IDamagebIble
{
    [Header("Stats")]
    public int health;
    ...
    {
        if (playerDistance < attackDistance && IsPlayerInFidleOfView())
        {
            agent.isStopped = true;
            if (Time.time - lastAttackTime > attackRate)
            {
                lastAttackTime = Time.time;
                CharacterManager.Instance.Player.controller.GetComponent<IDamagebIble>().TakePhsycialDamage(damage);
                animator.speed = 1;
